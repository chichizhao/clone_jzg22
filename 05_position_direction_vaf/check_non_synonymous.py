...
# here we based on the mutation site happend order and the population vcf file to check the mutation site whether it is a non-synonymous mutation
# FUNCTION: check the mutation site individual by individual, and check whether it is a non-synonymous mutation
# INPUT: the population vcf file, the mutation structure tree file, the gene annotation file, reference genome
# OUTPUT: the non-synonymous mutation site file individual by individual in column
# Usage: python3 non_synonymous_check.py -vcf solid_somatic_mutation.vcf -tree typha_somatic_mutation_tree.txt -gff annotation.gff3 -mut_type mutation_type.txt -ref typha.fa -out non_synonymous_mutation
