            ...
            if line.startswith("#"):
                ...
                for i in range(sample_num):
                    ...
                    if line[i+9].startswith("./.:.:") :
                        gts.append(0)
                    # here we got one case that may not fit this condition.
                    # the variant read at low frequencing the genetype would be like ./. or 0/0. but the flowing read with low frequency would be 1/1
                    # so we can not give the read 0, we need to give the read 1
                    else:
                        #if i == 26:
                        #    test +=1
                ...
                chr = line[0]
                pos = line[1]
                if chr + "_" + pos not in same_vcf_pop.keys() and chr + "_" + pos not in sample_vcf.keys():
                    ...
                    k =k+1
                    if all(gt == 1 for gt in gts):
                        same_vcf_pop[chr + "_" + pos+str(k)] = gts
                    else:
                        sample_vcf[chr + "_" + pos+str(k)] = gts
                for i in range(sample_num):
                    for j in range(i, sample_num):
...
def build_tree(same_num, diff_num, sample_name, same_vcf, diff_vcf, same_vcf_pop, sample_vcf):
    ...
    print(leaf_length, pos_sample_a, pos_sample_b)
